...
import Userinfo from "../modules/Tetris/UserinfoModel";
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(ACTIONS.GET_LEADERBOARD, async ({}) => {
      try {
        ...
        const showInfo = await Userinfo.find({})
          .sort({ totalScore: -1 })
          .limit(5);
        const tetrisInfo = await Tetris.find({})
          .sort({ updatedAt: -1 })
          .limit(15);
    ...
    socket.on(ACTIONS.GET_RESULT_TETRIS, async ({ tetrisId, gameScore }) => {
      try {
        const currentTetris = await Tetris.findByIdAndUpdate(tetrisId, {
          score: gameScore,
        });
        ...
        // if (currentTetris.level === 1 && score >= currentTetris.goal) {
        //   // Execute a message on a smart contract
        //   transferAmount = {
        //     amount: Math.floor(currentTetris.amount * 1.1 * 1e6).toString(),
        //     denom: "usei",
        //   };
        ...
        // if (currentTetris.level === 2 && score >= currentTetris.goal) {
        //   // Execute a message on a smart contract
        //   transferAmount = {
        //     amount: Math.floor(currentTetris.amount * 1.3 * 1e6).toString(),
        //     denom: "usei",
        //   };
        //   console.log(transferAmount);
        //   const fee = calculateFee(150000, "0.1usei");
        ...
        // if (currentTetris.level === 3 && score >= currentTetris.goal) {
        //   // Execute a message on a smart contract
        //   transferAmount = {
        //     amount: Math.floor(currentTetris.amount * 2 * 1e6).toString(),
        //     denom: "usei",
        //   };
        //   console.log(transferAmount);
        //   const fee = calculateFee(150000, "0.1usei");
        ...
        const numberofUser = await Userinfo.countDocuments();
        if (existingUser) {
          // Update the total score for the existing user
          existingUser.lastScore = gameScore;
          existingUser.totalScore += gameScore;
          existingUser.totalBalance += addBalance;
